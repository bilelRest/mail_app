...
public interface MailRepo extends JpaRepository<MailEntity,Long> {
    ...
    //    @Query("SELECT DISTINCT m.sender FROM MailEntity m WHERE m.mailUser = :user GROUP BY m.sender, m.id")
//    Page<MailEntity> getBypageableSender(@Param("user") Users user,Pageable pageable);
